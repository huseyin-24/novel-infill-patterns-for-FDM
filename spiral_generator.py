...
def circle_spiral(radius, num_points_spiral, num_turns):
    center = (107.5, 107.5)        # Define the center 
    ...
    # The offset walls two times
    for j in range(2): 
        offset_points = []
        k = (1- (j+1)* 0.18)
        for i in range(int(num_points*k)):
            # Calculate angle in radians
            angle = 2 * math.pi * i / (num_points*k)
            offset_x = center[0] + (radius- (j+1)*offset_distance ) * math.cos(angle)
            offset_y = center[1] + (radius- (j+1)*offset_distance ) * math.sin(angle)
            offset_points.append((offset_x, offset_y))
        offset_points.append(offset_points[0])
        walls_points = walls_points + offset_points

    # add spiral infill
    infill_points = []
    t = np.linspace(0, - num_turns * 2 * np.pi, num_points_spiral)
    r = np.linspace(0, radius - 2 * offset_distance, num_points_spiral)
    x = r * np.cos(t) + center[0]
    y = r * np.sin(t) + center[1]
    infill_points = [(xi, yi) for xi, yi in zip(x, y)]
    infill_points.reverse()
    infill_points = infill_points[: int(len(infill_points) * 0.9)]
    infill_points.append(center)
...
spiral_layer = circle_spiral(12.5, 250, 13)          # Spirally filled circular layer used in middle region of the sample
...
for layer in range(1050):             # Iterate over 1050 layers
    ...
    # middle region
        ...
        wall_points, infill_points = spiral_layer # Retrieve points to write gcode 
        wall_points = wall_points.tolist()        
        infill_points = infill_points.tolist()
